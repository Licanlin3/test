...
<script>
	...
	export default{
		...
		metaInfo(){
			...
			return {
				title: title + " -blog",
				meta: [{vmid: "description",name: "description",content: title + " -blog"}]
			}
		},
		...
		methods: {
			...
			share: function(type,url){
				let	title = document.title + " 这是一个 个人博客",
				...
				switch (type){
					case "QQ" : 
					_href = url + "?title=" + title +"&url=" + _url + "&desc=我分享了一篇文章，快来看看哦~" 
					break
					case "qzone" : 
					_href = url + "?title=" + title + "&url=" + _url + "&desc=我分享了一篇文章，快来看看哦~"  + "summary="
					break
					case "weixin" : 
					_href = url + "&url=" + _url
					break
